...
void sha256(const uint8_t* data, size_t n, uint8_t out32[32]) {
  ...
  if (n) memcpy(msg.data(), data, n);
  msg[n] = 0x80;
  for (int i = 0; i < 8; ++i) msg[newlen + 7 - i] = (uint8_t)(bitlen >> (i * 8));
  for (size_t off = 0; off < newlen + 8; off += 64) {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i)
...
std::string sha256_hex(const uint8_t* data, size_t n) {
  uint8_t h[32]; sha256(data, n, h);
  char buf[65]; for (int i=0;i<32;++i) snprintf(buf+i*2,3,"%02x",h[i]);
  return std::string(buf,64);
}
std::string sha256_hex(const std::vector<uint8_t>& v) {
  return v.empty() ? sha256_hex(nullptr,0) : sha256_hex(v.data(), v.size());
}
